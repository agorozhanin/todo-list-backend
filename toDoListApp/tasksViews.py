...
class TaskView(View):
    ...
    def post(self, request):
        ...
        if folder_id is None:
            ...
            flag_to_null_folder = True

        elif Folder.objects.filter(folder_id=folder_id):
            task_data = {
                'task_name': task_name,
                'task_description': task_description,
                'task_status': task_status,
            ...
            data = {
                'message': 'New task object has not been created because folder with folder_id is not exist'
            }
        ...
        if task_name is not None and task_status is not None:
            ...
            if is_status(task_status) and is_correct_name(task_name):
                ...
                folder = task_object.folder_id if flag_to_null_folder else task_object.folder_id.folder_id
                data = {
        ...
        if Task.objects.filter(task_id=task_id):
            ...
            if folder_id is not None:
                if Folder.objects.filter(folder_id=folder_id):
                    ...
                    data = {
                        'message': 'Task object has not been changed because folder with this folder_id is not exist'
                    }
                    return JsonResponse(data, status=404)
            else:
                task.folder_id = None
